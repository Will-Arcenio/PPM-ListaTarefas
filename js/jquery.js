	...
	para (; i <comprimento; i ++) {
		...
		if ((opções = argumentos [i])! = nulo) {
			...
			para (nome em opções) {
				...
				// Previne a poluição de Object.prototype
...
var i,
	...
	booleans = "verificado | selecionado | assíncrono | foco automático | reprodução automática | controles | adiar | desativado | oculto |" +
...
função Sizzle (seletor, contexto, resultados, semente) {
	...
	// Tente encontrar operações de atalho (em oposição a filtros) em documentos HTML
	if (! seed) {
...
setDocument = Sizzle.setDocument = function (node) {
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		assert (função (el) {
			...
			// O antigo Firefox não lança um identificador com escape incorreto.
...
Expr = Sizzle.selectors = {
	...
	filtro: {
		...
		"PSEUDO": função (pseudo, argumento) {
			...
			// Priorizar diferenciando maiúsculas de minúsculas, caso pseudos personalizados sejam adicionados com letras maiúsculas
	...
	para (; i <len; i ++) {
		if ((matcher = Expr.relative [tokens [i] .type])) {
			...
			if (matcher [expando]) {
				...
				// Encontre o próximo operador relativo (se houver) para o manuseio adequado
				j = ++ i;
				para (; j <len; j ++) {
...
function adoptValue (valor, resolver, rejeitar, noValue) {
	...
	// Uma vez que jQuery.when não desembrulhará então, podemos pular as verificações extras que aparecem em
	...
	Adiado: function (func) {
		var tuples = [
			...
			promessa = {
				...
				// Faça uma promessa para este adiado
...
Data.prototype = {
	...
	cache: função (proprietário) {
		...
		valor var = proprietário [this.expando];
...
função AdjustCSS (elem, prop, valueParts, tween) {
	...
	if (initialInUnit && initialInUnit [3]! == unidade) {
		...
		// Suporte: Firefox <= 54
		// Divida o valor alvo da iteração pela metade para evitar a interferência dos limites superiores do CSS (gh-2144)
		inicial = inicial / 2;
		...
		while (maxIterations--) {
			...
			// Avalie e atualize nossa melhor estimativa (duplicando as estimativas que zerarão).
...
function boxModelAdjustment (elem, dimensão, caixa, isBorderBox, estilos, computedVal) {
	...
	para (; i <4; i + = 2) {
		...
		if (! isBorderBox) {
			...
			// Mas ainda manter o controle de outra forma
			} outro {
	...
	if (! isBorderBox && computedVal> = 0) {
		...
		// Assumindo a medianiz de rolagem de número inteiro, subtrair o resto e arredondar para baixo
...
function getWidthOrHeight (elem, dimensão, extra) {
	...
	var styles = getStyles (elem),
		...
		// Falsifique a caixa de conteúdo até que saibamos que é necessário saber o verdadeiro valor.
...
jQuery.extend ({
	...
	css: função (elem, nome, extra, estilos) {
		...
		// Se um gancho foi fornecido, obtém o valor calculado a partir daí
	...
	jQuery.cssHooks [dimensão] = {
		...
		conjunto: função (elem, valor, extra) {
			var corresponde,
				...
				// Para evitar forçar um refluxo, busque boxSizing apenas se necessário (gh-3991)
			...
			// falsificando uma content-box para obter borda e preenchimento (gh-3699)
			...
			if (subtrair && (corresponde a rcssNum.exec (valor)) &&
	...
	jQuery.cssHooks [prefixo + sufixo] = {
		expandir: função (valor) {
			var i = 0,
				...
				parts = typeof value === "string"? valor.split (""): [valor];
...
função Tween (elem, opções, prop, fim, atenuação) {
	retornar novo Tween.prototype.init (elem, opções, prop, fim, atenuação);
}
jQuery.Tween = Tween;
	...
	run: function (percent) {
		...
		if (this.options.duration) {
			this.pos = eased = jQuery.easing [this.easing] (
				por cento, this.options.duration * percent, 0, 1, this.options.duration
			);
		} outro {
			this.pos = atenuado = porcentagem;
		}
	...
	_predefinição: {
		...
		set: function (tween) {
			...
			if (jQuery.fx.step [tween.prop]) {
				jQuery.fx.step [tween.prop] (interpolação);
...
function schedule () {
	if (inProgress) {
		if (document.hidden === false && window.requestAnimationFrame) {
			...
			window.setTimeout (agendar, jQuery.fx.interval);
		}
...
function defaultPrefilter (elem, props, opts) {
	...
	if (isBox && elem.nodeType === 1) {
		...
		// de overflowX e overflowY com valores idênticos e Edge apenas espelhos
		// o valor de overflowX lá.
